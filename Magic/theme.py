...
def read_theme() -> tuple:
    ...
    try:
        with open((os.getcwd() + "/resources/ initial.elsa")) as f: datas = f.read()
        colours = datas.split(";")
...
def new_background_colour(event = "") -> None:
    ...
    if (color := askcolor()[1]) is not None: theme_writer(color, text_color, button_colour)


def new_font_colour(event = "") -> None:
    """To add the new font colour"""
    bg_colour, text_color, button_colour = read_theme()
    if (color := askcolor()[1]) is not None: theme_writer(bg_colour, color, button_colour)


def new_button_colour(event = "") -> None:
    """To add the new button colour"""
    bg_colour, text_color, button_colour = read_theme()
    if (color := askcolor()[1]) is not None: theme_writer(bg_colour, text_color, color)
