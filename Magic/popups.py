...
def resetelsapopup():
    ...
    tkinterlib.tkinter_initialise(
        popups, x=int(screen_width / 2), y=int(screen_height / 2)
    )
    popups.geometry(f"193x50+{int(screen_width/2)}+{int(screen_height/2)}")
    talk("Are you sure that you want to reset Elsa")
    Label(
        popups, text="Are you sure you want to reset Elsa?", fg=text_color, bg=bg_colour
    ).place(x=0, y=0)
    ...
    Yes = Button(popups, text="Yes", bg=button_colour, fg=text_color, command=Yes)
